...
class Stocks(object):
    def __init__(self, position_json=None) -> None:
        ...
        self.avg_cost_basis = position_json.get('averagePrice')
        self.stock_price = position_json.get('marketValue') / self.quantity
        self.percent_day_change = position_json.get(
    ...
    def initialize_from_quote_json(ticker, quote_json):
        ...
        stock.quantity = 0
        stock.percent_day_change = quote_json[ticker].get(
    ...
    def get_price_history(self, client, start_date, end_date) -> dict:
        ...
        self.price_history_df = pd.DataFrame.from_dict(
            self.price_history['candles'])
        # needs to convert the timestamp to datetime;
        self.price_history_df['date'] = self.price_history_df['datetime'].apply(
            lambda x: datetime.fromtimestamp(x / 1000).date())
        self.price_history_df = self.price_history_df[PRICE_HISTORY_COLUMNS].sort_values(
        ...
        self.percent_week_change = 100 * \
            (self.stock_price - last_week_average_close_price) / \
            last_week_average_close_price
        self.percent_month_change = 100 * \
            (self.stock_price - last_month_average_close_price) / \
            last_month_average_close_price
