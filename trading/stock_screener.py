...
class StockScreener:
    def __init__(self, client, tickers_to_scan) -> None:
        ...
        for ticker in self.tickers_to_scan:
            try:
                ...
                stock.get_price_history(client, datetime.datetime.now(
    ...
    def day_change_larger_than_x_percent(self, x_percent: float) -> dict:
        stock_candidates = [stock for stock in self.stocks if abs(
            stock.percent_day_change) > x_percent]
        put_sell_candidates = [
            stock.ticker for stock in stock_candidates if stock.percent_day_change < 0]
        call_sell_candidates = [
            stock.ticker for stock in stock_candidates if stock.percent_day_change > 0]
    ...
    def week_change_larger_than_x_percent(self, x_percent: float) -> dict:
        stock_candidates = [stock for stock in self.stocks if abs(
            stock.percent_week_change) > x_percent]
        put_sell_candidates = [
            stock.ticker for stock in stock_candidates if stock.percent_week_change < 0]
        call_sell_candidates = [
            stock.ticker for stock in stock_candidates if stock.percent_week_change > 0]
        self.put_sell_candidates.update(set(put_sell_candidates))
        self.call_sell_candidates.update(set(call_sell_candidates))
    ...
    def month_change_larger_than_x_percent(self, x_percent: float) -> dict:
        stock_candidates = [stock for stock in self.stocks if abs(
            stock.percent_month_change) > x_percent]
        put_sell_candidates = [
            stock.ticker for stock in stock_candidates if stock.percent_month_change < 0]
        call_sell_candidates = [
            stock.ticker for stock in stock_candidates if stock.percent_month_change > 0]
        self.put_sell_candidates.update(set(put_sell_candidates))
        self.call_sell_candidates.update(set(call_sell_candidates))
    ...
    def run(self, day_change=5, week_change=10, month_change=20) -> dict:
        tickers_to_sell_option_dict = self.day_change_larger_than_x_percent(
            x_percent=day_change)
        print(f"Day change > {day_change}%, {tickers_to_sell_option_dict}")
        tickers_to_sell_option_dict = self.week_change_larger_than_x_percent(
            x_percent=week_change)
        print(f"Week change > {week_change}%, {tickers_to_sell_option_dict}")
        tickers_to_sell_option_dict = self.month_change_larger_than_x_percent(
            x_percent=month_change)
